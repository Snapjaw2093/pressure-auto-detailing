...
6. **Verify it worked** by submitting the booking form on the live site with test details, then checking that the message arrives on Telegram.
...
`logo.svg` is the site logo/favicon. `api/` holds the serverless functions: `create-checkout-session.js` (deposit amount and service list for Stripe) and `send-booking-request.js` (message template for booking requests), which uses the shared `_telegram.js` helper to actually send (the `_` prefix keeps Vercel from treating it as its own route).
